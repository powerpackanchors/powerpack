...
const ChoreographersPage = ({ onArtistClick }) => {
  ...
  useEffect(() => {
    ...
    const cachedArtists = getCached('artists_choreographers');
    const cachedReels = getCached('reels_choreographers');
    ...
    Promise.all([
      ...
      .then(([artistsData, reelsData]) => {
        ...
        setReels(reelsData);
        setCached('artists_choreographers', artistsData);
        setCached('reels_choreographers', reelsData);
        setLoading(false);
      })
      .catch(err => {
        console.error('Error fetching Choreographers data:', err);
        setLoading(false);
      });
  }, []);
  ...
  if (loading) {
    return (
      <div style={{ paddingTop: '80px' }}>
        <Categories activeCategory="choreographers" onCategoryChange={handleCategoryChange} />
  ...
  return (
    <div style={{ paddingTop: '80px' }}>
      ...
      <ChoreographerSection artists={artists} onArtistClick={onArtistClick} showViewAll={false} />
      
      <section className="gallery-section">
        <div className="section-container">
          <div className="gallery-header">
            <h2 className="gallery-title">Choreography Reels</h2>
          </div>
